...
import { codexRequest, type CodexReply } from '@/services/codex'
...
export function MeetingAssistPanel({ onBusy }: { onBusy: (busy: boolean) => void }) {
  ...
  useEffect(() => {
    try { localStorage.setItem(MEETING_PREFERENCES_KEY, JSON.stringify(preferences)) } catch { /* unavailable storage */ }
  }, [preferences])
  const [explaining, setExplaining] = useState(false)
  const [answer, setAnswer] = useState<{ target: string; context: string; result: CodexReply } | null>(null)
  ...
  useEffect(() => {
    ...
    const keydown = (event: KeyboardEvent) => {
      ...
      if (shortcutCaptureRef.current) {
        ...
        if (shortcut) {
          setPreferences((current) => ({ ...current, shortcut }))
  ...
  const explain = async (snapshot: MeetingExcerpt) => {
    ...
    try {
      const response = await codexRequest<{ target: string; result: CodexReply }>(settings.serverUrl, '/explanations', {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, signal: controller.signal,
        body: JSON.stringify({ target: frozen.target, preceding_context: context, recent_excerpt: frozen.recent || '', focus: frozen.focus || 'target', preset_prompt: current.presetPrompt, focus_points: current.focusPoints, lookback_seconds: current.lookbackSeconds, recent_seconds: current.recentSeconds, recent_weight: current.recentWeight, model: settings.codexModel || undefined, effort: settings.codexEffort || undefined }),
      })
      if (!alive.current) return
      setAnswer({ ...response, context })
      if (response.result.status !== 'completed') setError(response.result.error || '解释未完成，请重试。')
    } catch (cause) {
      if (alive.current) setError(cause instanceof Error ? cause.message : '解释失败')
    } finally {
  ...
  return <div className="meeting-assist">
    ...
    <p>触发时回看之前 {preferences.lookbackSeconds} 秒的会议内容，立即交给 Agent 总结和解释。回复以文字显示，旁听不中断。</p>
    ...
    <label>本次解释原文（可修改）<textarea aria-label="本次解释原文" value={excerpt.target} rows={4} maxLength={12000} disabled={explaining} onChange={(event) => setExcerpt((current) => ({ ...current, target: event.target.value, focus: undefined, recent: undefined }))} /></label>
    ...
    {answer && <article className="agent-message assistant meeting-answer"><strong>针对这段原话的解释</strong><blockquote>{answer.target}</blockquote><p>{answer.result.text || answer.result.error}</p><small>{answer.context ? '本次附带了前文' : '本次仅解释原文'} · {answer.result.usage ? `${answer.result.usage.total_tokens} tokens` : '用量暂不可用'}</small></article>}
    <fieldset className="meeting-preferences">
      <legend>解释与触发设置（自动保存在本机）</legend>
      <div className="meeting-preference-grid">
        <label>回看时长（秒）<input aria-label="回看时长（秒）" type="number" min={10} max={900} value={preferences.lookbackSeconds} onChange={(e) => updatePreference('lookbackSeconds', Math.max(10, Math.min(900, Math.round(Number(e.target.value)) || 120)))} /></label>
        <label>重点关注末尾（秒）<input aria-label="重点关注末尾（秒）" type="number" min={5} max={preferences.lookbackSeconds} value={preferences.recentSeconds} onChange={(e) => updatePreference('recentSeconds', Math.max(5, Math.min(preferences.lookbackSeconds, Math.round(Number(e.target.value)) || 30)))} /></label>
      ...
      <small>回看按 ASR 文字首次出现时间近似截取，原文最多 12,000 字符；末尾重点最多 2,000 字符。关注程度会写入 Agent 提示，不是精确的模型权重。</small>
