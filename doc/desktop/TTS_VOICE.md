...
- `语音转 TTS`：录音按钮先在前端录完整音频，停止后上传到 `/v1/tts/higgs/audio-to-speech`。后端对完整音频执行 ASR，再调用 Higgs TTS。上传音频也走同一个接口。
...
`语音转 TTS` 的录音与桌面语音识别共用稳定录音器：空闲时预热真实麦克风，开始后优先采集连续 PCM 并封装为 WAV，停止时立刻把状态浮窗切到 `thinking`。该模式只取麦克风输入：relay 未启用时直接使用设置中选定的输入设备，relay 已启用时克隆 relay 内部的输入麦克风轨道，不从输出混音总线取音，避免把 TTS/系统输出叠回 ASR 输入。麦克风启动或停止超时会清理轨道并隐藏浮窗，不会停留在“语音输入中”。
...
2. 实时 ASR、一句话离线录音和 `语音转 TTS` 需要同一麦克风时，从中转器取得输入音轨 clone；clone 只来自真实麦克风输入，不包含注入音效、TTS 或输出设备回环。
