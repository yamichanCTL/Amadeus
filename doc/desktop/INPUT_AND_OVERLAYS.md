...
Electron 当前最小窗口为 720×520。初始非全屏窗口使用主显示器完整工作区高度；小于 1440 px 的工作区使用全宽，更大的工作区使用 90% 宽度并水平居中。页面在 1320px、760px 和 560px 高度断点重排侧栏、表单和操作区；历史页额外按内容容器 1080px 断点切换双栏/单栏，筛选器允许换行，所有 grid 子项都可收缩。Windows E2E 覆盖 1280×720、1280×960 和 720×520，要求无横向溢出和列表/详情重叠。
