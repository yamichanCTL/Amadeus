...
1. 录音浮窗显示前，renderer 同时记录 Amadeus 内当前聚焦的 `input` / `textarea` 及光标选区，并调用 `text:captureTarget` 记录外部前台窗口句柄。ASR 最终文本一返回，renderer 先同步更新 Amadeus 当前结果和历史；如果录音开始时焦点位于本软件 Prompt 等可编辑框，会优先按原光标位置写入并触发 React `input` 更新，不再绕到 Windows UIAutomation。
...
连续离线识别使用 latest-wins 注入调度：如果前一轮 UI Automation/helper 仍卡住，新结果会先取消旧注入并立即执行；尚未开始的中间项不会继续阻塞最新文本。跨应用注入不再在 Electron main process 调用同步 `clipboard.writeText`，剪贴板写入只由常驻 STA helper 执行；结果弹窗复制则先立刻通知主进程隐藏窗口，再在独立 overlay renderer 的下一事件循环写剪贴板。因此即使其他程序持有 `OpenClipboard` 锁，主窗口、音频 IPC 和托盘也不会一起冻结，复制按钮和旁边的 `×` 都会关闭结果框。每个 pending 请求绑定其具体 helper，旧进程稍后退出也不能清理新请求。

应用或电脑重启后若常驻 PowerShell/UIAutomation helper 在 1.5 秒内未完成 `ready` 握手、管道未就绪、进程提前退出或首次请求超时，主进程会清理旧 helper 并自动重建后重试一次。主进程不再预写剪贴板；helper 在自身 STA 线程中写入后再粘贴。焦点明确不可编辑时不重试，已被更新 ASR 结果取消的旧请求也不会复活，避免把旧文本粘贴到新目标。
...
设置页可实时调整是否显示、字号、颜色、透明度、宽度、高度和默认位置；字幕正文区域使用 Electron 原生 drag region，按住任意非按钮区域即可移动，右上角按钮使用 no-drag 保持可点击。预览开启后控件变化立即调用 overlay 更新，拖动/缩放字幕框产生的 bounds 会持久化。
