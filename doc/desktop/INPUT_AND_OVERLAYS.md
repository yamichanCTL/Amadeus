...
1. 录音浮窗显示前，renderer 调用 `text:captureTarget`，Windows 主进程记录当时的前台窗口句柄。ASR 最终文本一返回，renderer 先同步更新 Amadeus 当前结果和历史，再启动外部投递 promise；UIAutomation 卡住不会阻止软件内结果显示。
...
连续离线识别使用 latest-wins 注入调度：如果前一轮 UI Automation/helper 仍卡住，新结果会先取消旧注入并立即执行；尚未开始的中间项不会继续阻塞最新文本。结果弹窗的“复制”不再让 Electron 主进程同步访问系统剪贴板：复制任务在独立 overlay renderer 的下一事件循环执行，主窗口、音频 IPC 和托盘不会被慢速 Windows 剪贴板 owner 一起冻结。每个 pending 请求绑定其具体 helper，旧进程稍后退出也不能清理新请求。
...
设置页可实时调整是否显示、字号、颜色、透明度、宽度、高度和默认位置；预览开启后控件变化立即调用 overlay 更新，拖动/缩放字幕框产生的 bounds 仍会持久化。
