...
启用应用级音频中转时不再额外打开预热流。离线快捷录音和 TTS 一句话录音仍然只采集麦克风输入：relay 未启用时直接打开设置中选定的输入设备，relay 已启用时从 `AudioRelayMixer` 克隆其内部输入麦克风轨道；该轨道不会连接输出/虚拟声卡混音总线。实时字幕和 Agent 免按键识别同样可从 `AudioRelayMixer` 克隆轨道，以便保持全双工实时链路。
