...
- 回看时长默认 120 秒，可设 10–900 秒，原文最多 12000 字符。
- 末尾重点默认 30 秒，可设 5 秒至回看时长，最多 2000 字符。
...
当前转写最多保留 64000 字符。时间窗使用 ASR 文字首次出现时间近似计算，修订保留既有时间；
...
`target` 必填、最多 12000 字符；`recent_excerpt` 必须是原文中的片段。
